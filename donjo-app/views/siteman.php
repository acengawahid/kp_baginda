...
<html>
	<head>
		...
		<?php endif; ?>
		<?php if (is_file(LOKASI_LOGO_DESA ."desa.png")): ?>
			<link rel="shortcut icon" href="<?= base_url()?><?=LOKASI_LOGO_DESA?>desa.png" />
		<?php else: ?>
			<link rel="shortcut icon" href="<?= base_url()?>desa.png" />
		<?php endif; ?>
	</head>
	<body class="login">
				...
				<div class="container">
					<div class="row">
						<div class="col-sm-6 col-sm-offset-3 form-box">
							<div class="form-top">
								<a href="<?=site_url(); ?>first/"><img src="<?=LogoDesa($desa['logo']);?>" alt="<?=$desa['nama_desa']?>" class="img-responsive" width="120" height="120"/></a>
							...
							<div class="form-bottom">
								<form class="login-form" action="<?=site_url('siteman/auth')?>" method="post" >
									<?php if ($_SESSION['siteman_wait']==1): ?>
										...
										<p style="color:red; text-transform:uppercase">Anda Gagal 3 kali, Silakan coba kembali dalam <?= waktu_ind((time()- $_SESSION['siteman_timeout'])*(-1));?> lagi</p>
										</div>
									<?php else: ?>
								...
								<hr/>
								<div class="login-footer-bottom">Designed by: <a href="#">Kerja Praktek | STMIK Sumedang</a> <br> powered by: <a href="#">OpenSID</a> <?= substr(AmbilVersi(), 0, 11)?></div>
							</div>
						</div>
					</div>
				</div>
